...
sys.path.append('./infer/yolov5')
# pprint.pprint(sys.path)
from detect_1img import run
weight_path = './infer/yolov5/exp21_best.pt'

sys.path.append('./infer/maml')
from load_model_predict import main
import  argparse
...
def create_patches_from_ptlist(ptlist, fname, path_patch, img_fore):
    ...
    for i, pt in enumerate(ptlist):
        # img = cv2.imread('../../data/foreground/' + fname.replace('png', 'jpg'))
        ...
        for i, pt in enumerate(ptlist):
            ...
            for_synthe_patches = for_synthe_patches + extract_patches(img_, patch_name, C[:-1], theta)
            corners_list.append([fname, patch_name, cx, cy])
        create_synthe_pathches(for_synthe_patches, fname)
...
def create_synthe_pathches(for_synthe_patches, fname):
    path_synthe = './data/patch/synthe/'
    for i, patch in enumerate(for_synthe_patches):
        cv2.imwrite('./data/patch/for_synthe/' + fname.replace('.png', '') + '_' + str(i) + '.png', patch)
    cnt = 0
...
#         if i == len(contours)-1:
#             cv2.imwrite('./data/corner.png', img__)
    ...
    df_natural = pd.DataFrame(corners_list)
    df_natural.columns = ['fname', 'patch_name', 'cx', 'cy']
    #     df.to_csv('../data/csv/natural_patches.csv', header=True, index=False)
    pnames = sorted(os.listdir('./data/patch/synthe/'))
